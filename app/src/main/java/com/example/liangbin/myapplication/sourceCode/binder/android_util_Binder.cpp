...
//1.保存JNI层调用Java层Binder需要的一些信息;//2.注册Java层调用JNI层的函数
static int int_register_android_os_Binder(JNIEnv* env)
{
    //1.保存JNI层调用Java层Binder需要的一些信息
    //根据全路径找到Binder类在JNI中的jclass对象
    jclass clazz = FindClassOrDie(env, kBinderPathName);
    //gBinderOffsets是静态类对象，用来保存Binder类在JNI中的一些信息。
    //如成员函数execTransact的methodId，成员变量mObject的fieldId。
...
//1.保存BinderInternal在JNI层的一些信息，供JNI层调用Java层。2.注册JNI函数，供Java层调用JNI层。
static int int_register_android_os_BinderInternal(JNIEnv* env)
{
    //1.保存BinderInternal在JNI层的一些信息，供JNI层调用Java层。
    //找到BinderInternal类在JNI层的jclass对象
    jclass clazz = FindClassOrDie(env, kBinderInternalPathName);
    //在静态类中保存jclass对象和forceBinderGc成员函数的methodId.
    gBinderInternalOffsets.mClass = MakeGlobalRefOrDie(env, clazz);
    gBinderInternalOffsets.mForceGc = GetStaticMethodIDOrDie(env, clazz, "forceBinderGc", "()V");
    //2.注册JNI函数，供Java层调用JNI层。
    return RegisterMethodsOrDie(
...
//1.保存BinderProxy在JNI层的一些信息，供JNI层调用Java层BinderProxy；2.注册JNI层函数，供Java层调用JNI层。
static int int_register_android_os_BinderProxy(JNIEnv* env)
{
    //找到BinderProxy在JNI层的jclass对象
    clazz = FindClassOrDie(env, kBinderProxyPathName);
    //使用静态变量保存BinderProxy类的JNI层的jclass对象，成员函数init的methodId,sendDeathNotice的methodId,
    //成员变量mObject的fieldId,mSelf的fieldId,mOrgue的fieldId.
